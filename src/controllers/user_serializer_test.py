...
def test_created_at_is_iso_or_none():
    serialized = serialize_user(_row(created_at=datetime(2026, 1, 2, 3, 4, 5)))
...
# The envelope serializes every row, so a hash cannot slip through the list
# path either — the listing is the response that exposes the most users at once.
def test_the_list_envelope_serializes_every_row():
    response = format_user_list_response([_row(), _row(id=8)], total=2, page=1, per_page=10)
